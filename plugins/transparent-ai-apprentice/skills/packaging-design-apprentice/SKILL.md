...
5. **Open the Chinese mask workbench.** Use `create-transparent-sketch-overlay-kit.mjs --content-type image --backdrop <sample image>` and show the self-check issues beside the sample. Require at least one `change` region; let the teacher mark already-correct content as `protect` and spatial dependencies as `reference`. Wait for submission. Do not infer approval from opening or closing the mask.
6. **Apply Image2 local editing.** Send the original sample, exact change masks, protection masks, teacher text, and self-check issues to Image2 as a local edit. Enforce `mingtu_surgical_edit_contract_v1`: change only selected regions, preserve all unmarked content, compare before/after outside every mask, and reject unrelated changes. Whole-sample regeneration is a separate teacher-reviewed fallback, never the default response to a local issue.
