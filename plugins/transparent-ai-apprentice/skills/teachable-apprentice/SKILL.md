...
Treat a correction as a local patch by default, not permission to regenerate the whole artifact. Keep the three teaching surfaces separate: use `create_transparent_sketch_overlay_kit` for the restored engineering-image overlay, `create_office_text_mask_workbench` for Word or Excel content, and `create_engineering_software_mask_workbench` for exact software objects. Never add a content-type switch to the original image overlay. The two dedicated new workbenches bind marks to `change`, `protect`, or `reference`; their `mingtu_multimodal_surgical_mask_correction_v1` packets preserve every unmarked target and reject unrelated changes. Whole-artifact regeneration remains a separate teacher choice.
