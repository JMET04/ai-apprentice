...
Treat a correction as a local patch by default, not permission to regenerate the whole artifact. Keep the three teaching surfaces separate: use `create_transparent_sketch_overlay_kit` for the restored engineering-image overlay, `create_office_text_mask_workbench` for Word or Excel content, and `create_engineering_software_mask_workbench` for exact software objects. Never add a content-type switch to the original image overlay. The two dedicated new workbenches bind marks to `change`, `protect`, or `reference`; their `transparent_ai_apprentice_multimodal_surgical_mask_correction_v1` packets preserve every unmarked target and reject unrelated changes. Whole-artifact regeneration remains a separate teacher choice.

For Word and Excel text, the visual mask is only a pointer. Bind Word edits to `paragraph:N` and Excel edits to `SheetName!A1`, preserve the source file, and use `surgical-office-text-edit.py` to create a separate output plus `transparent_ai_apprentice_surgical_office_text_edit_report_v1`. Require an exact source-text match and prove that only `word/document.xml` or the selected worksheet XML changed. A layout-risk warning narrows any visual check to the changed paragraph or cell; do not make the teacher recheck the whole document.
...
The MCP server should keep the default `tools/list` teacher-facing. Use the bounded task surface only for agents that need explicit lower-level tools by setting `TRANSPARENT_AI_APPRENTICE_TOOL_MODE=advanced`; use `TRANSPARENT_AI_APPRENTICE_TOOL_MODE=full` only for maintainers and compatibility audits. Do not ask an ordinary teacher to choose between internal construction tools.
