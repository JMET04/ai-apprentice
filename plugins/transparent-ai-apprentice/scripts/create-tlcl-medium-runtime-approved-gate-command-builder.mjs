...
function slugify(value) {
  return (
    String(value || "tlcl-medium-runtime-approved-gate-command-builder")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "")
      .slice(0, 72) || "tlcl-medium-runtime-approved-gate-command-builder"
  );
}
...
const goal = argValue("--goal", argValue("--task", "Build a TLCL teacher-facing approved-gate command page."));
const prepInput = readJsonInput(
  argValue("--prep", argValue("--prep-runner", argValue("--tlcl-prep", ""))),
  "--prep",
...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "tlcl-medium-runtime-approved-gate-command-builders"))
);
...
const blockers = [];
if (prep.status !== "execution_approval_gate_prepared_waiting_for_teacher_execute_review") {
  blockers.push("tlcl_prep_status_not_ready_for_teacher_execute_review");
}
if (prep.validationReady !== true) blockers.push("tlcl_prep_validation_not_ready");
if (prep.approvalGateInvoked !== true) blockers.push("tlcl_prep_approval_gate_not_invoked");
if (prep.readyForExecuteRequest !== true) blockers.push("tlcl_prep_readyForExecuteRequest_not_true");
if (prep.locks?.prepRunnerDoesNotInvokeExecutionRunner !== true) blockers.push("tlcl_prep_execution_runner_lock_missing");
...
const wrapperPath = join(builderDir, "tlcl-medium-runtime-approved-gate-command-builder.json");
const receiptPath = join(builderDir, "tlcl-medium-runtime-approved-gate-command-builder-receipt.json");
const readmePath = join(builderDir, "TLCL_MEDIUM_RUNTIME_APPROVED_GATE_COMMAND_BUILDER_START_HERE.md");
const wrapper = {
  ok: true,
  format: "transparent_ai_tlcl_medium_runtime_approved_gate_command_builder_v1",
  builderId,
  ...
  blockers,
  blockedTransitions: [
    "run_approved_gate_runner_from_tlcl_builder",
    "execute_target_software_from_tlcl_builder",
    "send_ui_events_from_tlcl_builder",
    "capture_screenshot_from_tlcl_builder",
    "write_memory_from_tlcl_builder",
    "enable_rule_from_tlcl_builder",
    "unlock_packaging_from_tlcl_builder",
    "claim_completion_from_tlcl_builder"
  ],
  nextTeacherActions: blockers.length
    ? [
        "Resolve every blocker before using a final approved-gate command builder.",
        "Return to the TLCL approval gate prep runner if the source gate is missing or not ready."
      ]
    : [
        "Open the generated existing command-builder HTML.",
        "Review the ready approval gate and generated command request.",
...
const receipt = {
  ok: true,
  format: "transparent_ai_tlcl_medium_runtime_approved_gate_command_builder_receipt_v1",
  builderId,
  status,
...
writeFileSync(
  readmePath,
  [
    "# TLCL Medium Runtime Approved-Gate Command Builder",
    "",
    `Status: ${status}`,
    `Source prep: ${prepInput.path || "<inline>"}`,
    `Approval gate: ${approvalGatePath || "<missing>"}`,
    "",
    "This TLCL wrapper validates that a ready execution approval gate came from the TLCL medium-runtime prep chain, then reuses the existing approved-gate command builder.",
    "",
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_tlcl_medium_runtime_approved_gate_command_builder_result_v1",
      builderId,
      status,
      wrapperPath,
      receiptPath,
