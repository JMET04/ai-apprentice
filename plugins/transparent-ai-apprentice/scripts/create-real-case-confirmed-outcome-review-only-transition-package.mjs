...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-review-only-transition-packages"))
);
const teacherReviewed = hasFlag("--teacher-reviewed");

if (!planningPacketPath || !existsSync(planningPacketPath)) {
  throw new Error(`REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PLANNING_PACKET_NOT_FOUND: ${planningPacketPath || "<missing>"}`);
}
if (!teacherReviewed) throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_TRANSITION_PACKAGE_REQUIRES_TEACHER_REVIEWED_FLAG");
...
if (planningPacket.format !== "transparent_ai_real_case_confirmed_outcome_review_only_package_planning_packet_v1") {
  throw new Error("Expected transparent_ai_real_case_confirmed_outcome_review_only_package_planning_packet_v1.");
}
if (
...
if (planningPacket.confirmedOutcomeBranch !== true) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_TRANSITION_SOURCE_BRANCH_MISSING");
}
if (planningPacket.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_TRANSITION_SOURCE_FORMAT_MISMATCH");
}
if (!planningPacket.sourceConfirmedOutcomeReviewId || !planningPacket.sourceConfirmedOutcomeSourceRunId || !planningPacket.sourceRunId) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_TRANSITION_SOURCE_IDS_MISSING");
}
...
const transitionId = stableId("real_case_confirmed_outcome_review_only_transition_package", `${planningPacketPath}:${rollbackPoint}`);
...
const sourceContext = {
  ...
  sourceReviewFormat: planningPacket.sourceReviewFormat,
  sourceConfirmedOutcomeReviewId: planningPacket.sourceConfirmedOutcomeReviewId,
  sourceConfirmedOutcomeSourceRunId: planningPacket.sourceConfirmedOutcomeSourceRunId,
  sourceRunId: planningPacket.sourceRunId
};
...
if (errors.length) {
  const blocked = {
    ok: false,
    format: "transparent_ai_real_case_confirmed_outcome_review_only_transition_package_v1",
    transitionId,
    createdAt: new Date().toISOString(),
    status: "blocked_real_case_confirmed_outcome_review_only_transition_package",
    planningPacketPath,
  ...
  const blockedPath = join(transitionDir, "real-case-confirmed-outcome-review-only-transition-package.json");
...
if (activeRules.length) throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PACKAGE_CONTAINS_ACTIVE_RULES");
if (nonReviewOnlyRules.length) throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PACKAGE_CONTAINS_NON_REVIEW_ONLY_RULES");
...
const packetPath = join(transitionDir, "real-case-confirmed-outcome-review-only-transition-package.json");
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_real_case_confirmed_outcome_review_only_transition_package_result_v1",
