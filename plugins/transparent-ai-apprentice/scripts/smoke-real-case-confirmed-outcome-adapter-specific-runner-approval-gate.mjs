...
const dryRunSmokeRoot = join(root, ".ta-smoke", "real-case-confirmed-outcome-controlled-execution-dry-run");
const smokeRoot = join(root, ".ta-smoke", "real-case-confirmed-outcome-adapter-specific-runner-approval-gate");
...
const dryRunSmoke = JSON.parse(
  runNode(["plugins/transparent-ai-apprentice/scripts/smoke-real-case-confirmed-outcome-controlled-execution-request-dry-run.mjs"])
    .stdout
);
const dryRunPath = latestFile(
  join(dryRunSmokeRoot, "approved-dry-run"),
  "real-case-confirmed-outcome-controlled-execution-dry-run.json"
);
const receiptTemplatePath = latestFile(
  join(dryRunSmokeRoot, "approved-dry-run"),
  "real-case-confirmed-outcome-controlled-execution-dry-run-receipt-template.json"
);
...
const approvedReceiptPath = writeJson(join(smokeRoot, "approved-dry-run-receipt.json"), {
  ...
  adapterSelection: {
    ...
    adapterId: "drawio-confirmed-outcome-existing-tool-adapter",
  ...
  teacherNotes: "Approve only the confirmed-outcome adapter-specific gate for a later separate real runner."
});
const approvedResult = JSON.parse(
  runNode([
    "plugins/transparent-ai-apprentice/scripts/validate-real-case-confirmed-outcome-controlled-execution-dry-run-receipt.mjs",
    "--dry-run",
    dryRunPath,
    "--receipt",
    approvedReceiptPath,
...
check(
  ...
  approvedResult.format === "transparent_ai_real_case_confirmed_outcome_adapter_specific_runner_approval_gate_result_v1" &&
    approvedResult.status === "real_case_confirmed_outcome_adapter_specific_runner_approval_gate_ready_for_separate_real_runner" &&
    ...
    approvedResult.sourceConfirmedOutcomeReviewId === dryRun.sourceConfirmedOutcomeReviewId &&
    approvedResult.sourceConfirmedOutcomeSourceRunId === dryRun.sourceConfirmedOutcomeSourceRunId &&
    approvedResult.sourceRunId === dryRun.sourceRunId &&
    approvedResult.separateRealRunnerRequest?.format ===
      "transparent_ai_real_case_confirmed_outcome_separate_real_runner_request_v1" &&
    approvedResult.separateRealRunnerRequest?.sourceReviewFormat === expectedSourceReviewFormat &&
    approvedResult.separateRealRunnerRequest?.sourceConfirmedOutcomeReviewId === dryRun.sourceConfirmedOutcomeReviewId &&
    approvedResult.separateRealRunnerRequest?.sourceConfirmedOutcomeSourceRunId === dryRun.sourceConfirmedOutcomeSourceRunId &&
    approvedResult.separateRealRunnerRequest?.sourceRunId === dryRun.sourceRunId &&
    ...
    approvedGate.locks?.confirmedOutcomeBranch === true &&
    approvedGate.confirmedOutcomeBranch === true &&
    approvedGate.sourceReviewFormat === expectedSourceReviewFormat &&
    approvedGate.sourceConfirmedOutcomeReviewId === dryRun.sourceConfirmedOutcomeReviewId &&
    approvedGate.sourceConfirmedOutcomeSourceRunId === dryRun.sourceConfirmedOutcomeSourceRunId &&
    approvedGate.sourceRunId === dryRun.sourceRunId,
  JSON.stringify({ gatePath: approvedResult.gatePath })
);

const sourceTamperReceiptPath = writeJson(join(smokeRoot, "source-tamper-dry-run-receipt.json"), {
...
const sourceTamperResult = JSON.parse(
  runNode(
    [
      ...
      "--out-dir",
      join(smokeRoot, "source-tamper-gate")
    ],
    { expectOk: false }
  ).stdout
);
check(
  "Real-case confirmed outcome adapter-specific runner approval gate blocks lost confirmed-outcome source continuity",
    sourceTamperResult.status === "real_case_confirmed_outcome_adapter_specific_runner_gate_needs_teacher_review" &&
    ...
    sourceTamperResult.adapterInvoked === false &&
    sourceTamperResult.targetSoftwareCommandsExecuted === false,
  JSON.stringify({ blockers: sourceTamperResult.blockers })
);

const missingSourceRunDryRunPath = writeJson(join(smokeRoot, "missing-source-run-dry-run.json"), {
  ...dryRun,
...
const missingSourceRunResult = JSON.parse(
  runNode(
    [
      "plugins/transparent-ai-apprentice/scripts/validate-real-case-confirmed-outcome-controlled-execution-dry-run-receipt.mjs",
      "--dry-run",
      missingSourceRunDryRunPath,
      "--receipt",
      approvedReceiptPath,
      "--out-dir",
      join(smokeRoot, "missing-source-run-gate")
    ],
    { expectOk: false }
  ).stdout
);
check(
  "Real-case confirmed outcome adapter-specific runner approval gate blocks missing source run continuity",
  missingSourceRunResult.status === "real_case_confirmed_outcome_adapter_specific_runner_gate_needs_teacher_review" &&
    missingSourceRunResult.blockers.some((row) => row.code === "source_ids_missing") &&
    missingSourceRunResult.adapterInvoked === false &&
    missingSourceRunResult.targetSoftwareCommandsExecuted === false,
  JSON.stringify({ blockers: missingSourceRunResult.blockers })
);

const missingCurrentSourceRunDryRunPath = writeJson(join(smokeRoot, "missing-current-source-run-dry-run.json"), {
  ...dryRun,
  sourceRunId: ""
});
const missingCurrentSourceRunResult = JSON.parse(
  runNode(
    [
      "plugins/transparent-ai-apprentice/scripts/validate-real-case-confirmed-outcome-controlled-execution-dry-run-receipt.mjs",
      "--dry-run",
      missingCurrentSourceRunDryRunPath,
      "--receipt",
      approvedReceiptPath,
      "--out-dir",
...
check(
  "Real-case confirmed outcome adapter-specific runner approval gate blocks missing current sourceRunId",
  missingCurrentSourceRunResult.status === "real_case_confirmed_outcome_adapter_specific_runner_gate_needs_teacher_review" &&
    missingCurrentSourceRunResult.blockers.some((row) => row.code === "source_ids_missing") &&
    missingCurrentSourceRunResult.adapterInvoked === false &&
...
check(
  ...
  forbiddenResult.status === "blocked_for_forbidden_confirmed_outcome_runner_gate_decision" &&
...
check(
  ...
  missingAdapterResult.status === "real_case_confirmed_outcome_adapter_specific_runner_gate_needs_teacher_review" &&
...
check(
  ...
  tamperedResult.status === "real_case_confirmed_outcome_adapter_specific_runner_gate_needs_teacher_review" &&
...
const repairReceiptPath = writeJson(join(smokeRoot, "repair-receipt.json"), {
  ...
  teacherNotes: "Confirmed-outcome adapter choice is not precise enough; repair the gate before real runner."
});
...
check(
  ...
  repairResult.status === "real_case_confirmed_outcome_adapter_specific_runner_gate_routes_to_high_reasoning_repair" &&
...
const summary = {
  format: "transparent_ai_real_case_confirmed_outcome_adapter_specific_runner_approval_gate_smoke_v1",
