...
function normalizeExecutionGateValidation(input) {
  if (input.value.format === "transparent_ai_real_case_active_execution_gate_validation_result_v1") {
...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-controlled-execution-dry-runs"))
);
...
if (
  ![
    ...
    "transparent_ai_real_case_active_execution_gate_validation_result_v1"
...
if (validation.status !== "real_case_active_execution_gate_ready_for_controlled_execution_request") {
...
const dryRunPath = join(dryRunDir, "real-case-controlled-execution-dry-run.json");
const receiptTemplatePath = join(dryRunDir, "real-case-controlled-execution-dry-run-receipt-template.json");
const readmePath = join(dryRunDir, "REAL_CASE_CONTROLLED_EXECUTION_DRY_RUN_START_HERE.md");
const ok = blockers.length === 0;
const status = ok
  ? "real_case_controlled_execution_dry_run_ready_for_teacher_runner_review"
  : "real_case_controlled_execution_dry_run_blocked";
...
const dryRun = {
  ok,
  format: "transparent_ai_real_case_controlled_execution_dry_run_v1",
  dryRunId,
...
const receiptTemplate = {
  format: "transparent_ai_real_case_controlled_execution_dry_run_receipt_v1",
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Controlled Execution Dry-Run",
    "",
    `Status: ${status}`,
...
console.log(
  JSON.stringify(
    {
      ok,
      format: "transparent_ai_real_case_controlled_execution_dry_run_result_v1",
      status,
      dryRunPath,
      receiptTemplatePath,
      readmePath,
