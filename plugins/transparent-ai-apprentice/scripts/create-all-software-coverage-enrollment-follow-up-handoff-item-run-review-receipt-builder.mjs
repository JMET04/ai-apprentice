...
function slugify(value) {
  return (
    String(value || "coverage-enrollment-handoff-item-run-review-receipt-builder")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "")
      .slice(0, 72) || "coverage-enrollment-handoff-item-run-review-receipt-builder"
  );
}
...
function writeReadme(path, builder) {
  const lines = [
    "# Coverage Enrollment Handoff Item Run Review Receipt Builder",
    "",
    `Status: ${builder.status}`,
    `Selected follow-up: ${builder.runSummary.selectedFollowUpId || "none"}`,
    `Run status: ${builder.runSummary.status}`,
    `Review scope: ${builder.reviewScope?.scopeKind || "unspecified"}`,
    "",
    "Use the receipt template after one coverage enrollment follow-up handoff item runner has produced evidence.",
    "",
...
<head>
  ...
  <title>Coverage Enrollment Handoff Item Run Review</title>
  <style>
...
<body>
  <main>
    <h1>Coverage Enrollment Handoff Item Run Review</h1>
    <p><strong>Status:</strong> ${htmlEscape(builder.status)}</p>
    <p><strong>Selected follow-up:</strong> ${htmlEscape(builder.runSummary.selectedFollowUpId || "none")}</p>
    <p><strong>Run status:</strong> ${htmlEscape(builder.runSummary.status)}</p>
    <p><strong>Review scope:</strong> <code>${htmlEscape(builder.reviewScope?.scopeKind || "unspecified")}</code></p>
...
const goal = argValue("--goal", "Build a teacher review receipt for one coverage enrollment follow-up handoff item run.");
...
const runReceiptInput = readJsonInput(
  ...
  "--run-receipt",
  "transparent_ai_all_software_coverage_enrollment_follow_up_handoff_queue_item_run_receipt_v1"
);

const outputRoot = resolve(
  argValue(
    "--output-dir",
    join(process.cwd(), ".transparent-apprentice", "all-software-coverage-enrollment-follow-up-handoff-item-run-review-receipt-builders")
  )
);
...
const reviewScope = run.reviewScope || runReceipt.reviewScope || run.selectedItem?.reviewScope || null;
const builderPath = join(builderDir, "coverage-enrollment-handoff-item-run-review-receipt-builder.json");
const htmlPath = join(builderDir, "coverage-enrollment-handoff-item-run-review-receipt-builder.html");
const readmePath = join(builderDir, "COVERAGE_ENROLLMENT_HANDOFF_ITEM_RUN_REVIEW_RECEIPT_BUILDER_START_HERE.md");
const receiptTemplatePath = join(builderDir, "coverage-enrollment-handoff-item-run-review-receipt-template.json");
const selectedFollowUpId = run.selectedItem?.selectedFollowUpId || runReceipt.selectedItem?.selectedFollowUpId || "";
const runSummary = {
  ...
  batchPath: run.generatedEvidence?.batchPath || runReceipt.batchPath || "",
  batchReceiptPath: run.generatedEvidence?.batchReceiptPath || "",
  ranToolCount: run.generatedEvidence?.ranToolCount || 0,
  runResultStatuses: Array.isArray(run.generatedEvidence?.runResultStatuses) ? run.generatedEvidence.runResultStatuses : [],
...
const receiptTemplate = {
  format: "transparent_ai_all_software_coverage_enrollment_follow_up_handoff_item_run_review_receipt_v1",
  builderId,
...
const nextValidationCommand = commandLine("validate-all-software-coverage-enrollment-follow-up-handoff-item-run-review-receipt.mjs", [
  ["--builder", builderPath],
  ["--receipt", "<teacher-filled-coverage-enrollment-handoff-item-run-review-receipt.json>"]
]);
const builder = {
  ok: true,
  format: "transparent_ai_all_software_coverage_enrollment_follow_up_handoff_item_run_review_receipt_builder_v1",
  builderId,
  createdAt: new Date().toISOString(),
  goal,
  status: "coverage_enrollment_handoff_item_run_review_receipt_builder_ready",
  reviewScope,
  runSummary,
  receiptTemplate,
  nextValidationCommand,
  blockedActions: [
    "rerun_coverage_enrollment_handoff_item_from_builder",
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_all_software_coverage_enrollment_follow_up_handoff_item_run_review_receipt_builder_result_v1",
      builderId,
      status: builder.status,
      builderPath,
      htmlPath,
