...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-active-rule-package-compilations"))
);
...
if (!teacherReviewed) throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_RULE_PACKAGE_COMPILATION_REQUIRES_TEACHER_REVIEWED_FLAG");
...
if (
  validationFormat !== "transparent_ai_real_case_confirmed_outcome_active_promotion_review_validation_v1" &&
  validationFormat !== "transparent_ai_real_case_confirmed_outcome_active_promotion_review_validation_result_v1"
) {
  throw new Error("Expected transparent_ai_real_case_confirmed_outcome_active_promotion_review_validation_v1 or result_v1.");
}
if (
  validation.status !== "real_case_confirmed_outcome_active_promotion_review_ready_for_active_package_planning" ||
  validation.readyForActivePromotionPlanning !== true ||
  validation.activePromotionPlanningHandoff?.format !== "transparent_ai_real_case_confirmed_outcome_active_promotion_planning_handoff_v1" ||
...
if (transitionPackage.format !== "transparent_ai_real_case_confirmed_outcome_review_only_transition_package_v1") {
  throw new Error("Expected transparent_ai_real_case_confirmed_outcome_review_only_transition_package_v1.");
}
if (
...
if (
  validation.confirmedOutcomeBranch !== true ||
  handoff.confirmedOutcomeBranch !== true ||
  transitionPackage.confirmedOutcomeBranch !== true
) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_RULE_PACKAGE_SOURCE_BRANCH_MISSING");
}
if (
  validation.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT ||
  handoff.sourceReviewFormat !== validation.sourceReviewFormat ||
  transitionPackage.sourceReviewFormat !== validation.sourceReviewFormat
) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_RULE_PACKAGE_SOURCE_FORMAT_MISMATCH");
}
if (
  !validation.sourceConfirmedOutcomeReviewId ||
  !validation.sourceConfirmedOutcomeSourceRunId ||
  !validation.sourceRunId ||
  handoff.sourceConfirmedOutcomeReviewId !== validation.sourceConfirmedOutcomeReviewId ||
  handoff.sourceConfirmedOutcomeSourceRunId !== validation.sourceConfirmedOutcomeSourceRunId ||
  handoff.sourceRunId !== validation.sourceRunId ||
  transitionPackage.sourceConfirmedOutcomeReviewId !== validation.sourceConfirmedOutcomeReviewId ||
  transitionPackage.sourceConfirmedOutcomeSourceRunId !== validation.sourceConfirmedOutcomeSourceRunId ||
  transitionPackage.sourceRunId !== validation.sourceRunId
) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_RULE_PACKAGE_SOURCE_IDS_MISSING_OR_MISMATCHED");
}

const compilationId = stableId("real_case_confirmed_outcome_active_rule_package_compilation", `${validationInput.path || hashText(JSON.stringify(validation))}:${rollbackPoint}`);
...
if (errors.length) {
  const blocked = {
    ok: false,
    format: "transparent_ai_real_case_confirmed_outcome_active_rule_package_compilation_v1",
    compilationId,
  ...
  const blockedPath = join(compilationDir, "real-case-confirmed-outcome-active-rule-package-compilation.json");
...
if (!compiledRules.length) throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_RULE_PACKAGE_HAS_NO_RULES");
if (nonActiveRules.length) throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_RULE_PACKAGE_CONTAINS_NON_ACTIVE_RULES");
...
const packetPath = join(compilationDir, "real-case-confirmed-outcome-active-rule-package-compilation.json");
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_real_case_confirmed_outcome_active_rule_package_compilation_result_v1",
