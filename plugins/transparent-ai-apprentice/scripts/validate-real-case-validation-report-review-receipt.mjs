...
function slug(value) {
  return (
    String(value || "real-case-validation-report-review-validation")
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 72) || "real-case-validation-report-review-validation"
  );
}
...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-validation-report-review-validations"))
);
...
const status = forbiddenDecisionUsed
  ...
  : readyForLifecycleCandidate
    ...
    : routesToHighReasoningRepair
      ? "real_case_validation_report_review_routes_to_high_reasoning_repair"
      : routesToMoreEvidence
        ? "real_case_validation_report_review_waiting_for_more_evidence"
        : "real_case_validation_report_review_needs_teacher_review";

const validationId = `${new Date().toISOString().replace(/[:.]/g, "-")}-${slug(decision)}`;
const validationDir = join(outRoot, validationId);
const validationPath = join(validationDir, "real-case-validation-report-review-validation.json");
const receiptRecordPath = join(validationDir, "real-case-validation-report-review-receipt.json");
const readmePath = join(validationDir, "REAL_CASE_VALIDATION_REPORT_REVIEW_VALIDATION_START_HERE.md");
const validationLocks = locks();
const lifecycleCandidateHandoff = readyForLifecycleCandidate
  ? {
      format: "transparent_ai_real_case_lifecycle_candidate_handoff_v1",
...
const highReasoningRepairHandoff = routesToHighReasoningRepair
  ? {
      format: "transparent_ai_real_case_validation_report_high_reasoning_repair_handoff_v1",
      reportId: packet.reportId,
...
const moreEvidenceHandoff = routesToMoreEvidence
  ? {
      format: "transparent_ai_real_case_validation_report_more_evidence_handoff_v1",
      reportId: packet.reportId,
      teacherNotes: receipt.teacherNotes || "",
...
const result = {
  ...
  format: "transparent_ai_real_case_validation_report_review_validation_v1",
  validationId,
...
writeFileSync(
  readmePath,
  [
    "# Real Case Validation Report Review Validation",
    "",
    `Status: ${status}`,
    `Decision: ${decision}`,
    "",
...
console.log(
  JSON.stringify(
    {
      ok: result.ok,
      format: "transparent_ai_real_case_validation_report_review_validation_result_v1",
      status,
      validationPath,
      receiptRecordPath,
      readmePath,
