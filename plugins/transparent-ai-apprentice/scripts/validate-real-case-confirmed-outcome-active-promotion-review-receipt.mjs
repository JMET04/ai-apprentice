...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-active-promotion-review-validations"))
);
...
if (transitionPackage.confirmedOutcomeBranch !== true) {
  block("source_confirmed_outcome_branch_missing", "Transition package must preserve confirmedOutcomeBranch=true.");
}
if (transitionPackage.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT) {
  block("source_review_format_mismatch", "Transition package must preserve the confirmed-outcome source review format.");
}
if (!transitionPackage.sourceConfirmedOutcomeReviewId || !transitionPackage.sourceConfirmedOutcomeSourceRunId || !transitionPackage.sourceRunId) {
  block("source_ids_missing", "Transition package must preserve confirmed-outcome review, source run, and run ids.");
}
...
if (receipt.sourceReviewFormat !== transitionPackage.sourceReviewFormat) {
  block("receipt_source_review_format_mismatch", "Receipt sourceReviewFormat must match the transition package.");
}
if (receipt.sourceConfirmedOutcomeReviewId !== transitionPackage.sourceConfirmedOutcomeReviewId) {
  block("receipt_source_review_id_mismatch", "Receipt sourceConfirmedOutcomeReviewId must match the transition package.");
}
if (receipt.sourceConfirmedOutcomeSourceRunId !== transitionPackage.sourceConfirmedOutcomeSourceRunId) {
  block(
    "receipt_source_confirmed_outcome_source_run_id_mismatch",
    "Receipt sourceConfirmedOutcomeSourceRunId must match the transition package."
  );
}
if (receipt.sourceRunId !== transitionPackage.sourceRunId) {
  block("receipt_source_run_id_mismatch", "Receipt sourceRunId must match the transition package.");
}
  ...
  : readyForActivePromotionPlanning
    ...
    : routesToHighReasoningRepair
      ...
      : routesToMoreEvidence
        ? "real_case_confirmed_outcome_active_promotion_review_waiting_for_more_evidence"
        : "real_case_confirmed_outcome_active_promotion_review_needs_teacher_review";

const validationId = `${new Date().toISOString().replace(/[:.]/g, "-")}-${decision}`;
const validationDir = join(outRoot, validationId);
const validationPath = join(validationDir, "real-case-confirmed-outcome-active-promotion-review-validation.json");
const receiptRecordPath = join(validationDir, "real-case-confirmed-outcome-active-promotion-review-receipt.json");
const readmePath = join(validationDir, "REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_PROMOTION_REVIEW_VALIDATION_START_HERE.md");
const validationLocks = locks({ approved: readyForActivePromotionPlanning });
const sourceContext = {
  confirmedOutcomeBranch: transitionPackage.confirmedOutcomeBranch === true,
  sourceReviewFormat: transitionPackage.sourceReviewFormat,
  sourceConfirmedOutcomeReviewId: transitionPackage.sourceConfirmedOutcomeReviewId,
  sourceConfirmedOutcomeSourceRunId: transitionPackage.sourceConfirmedOutcomeSourceRunId,
  sourceRunId: transitionPackage.sourceRunId
};

const activePromotionPlanningHandoff = readyForActivePromotionPlanning
  ? {
      format: "transparent_ai_real_case_confirmed_outcome_active_promotion_planning_handoff_v1",
...
const highReasoningRepairHandoff = routesToHighReasoningRepair
  ? {
      format: "transparent_ai_real_case_confirmed_outcome_active_promotion_high_reasoning_repair_handoff_v1",
      transitionId: transitionPackage.transitionId || "",
      ...sourceContext,
...
const moreEvidenceHandoff = routesToMoreEvidence
  ? {
      format: "transparent_ai_real_case_confirmed_outcome_active_promotion_more_evidence_handoff_v1",
      transitionId: transitionPackage.transitionId || "",
      ...sourceContext,
      teacherNotes: receipt.teacherNotes || "",
...
const result = {
  ...
  format: "transparent_ai_real_case_confirmed_outcome_active_promotion_review_validation_v1",
  validationId,
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Confirmed Outcome Active Promotion Review Validation",
    "",
    `Status: ${status}`,
    `Decision: ${decision}`,
    "",
...
console.log(
  JSON.stringify(
    {
      ok: result.ok,
      format: "transparent_ai_real_case_confirmed_outcome_active_promotion_review_validation_result_v1",
      status,
      validationPath,
      receiptRecordPath,
      readmePath,
