...
const receiptInput = readJsonInput(
  ...
  "--receipt",
  "transparent_ai_real_case_confirmed_outcome_separate_real_runner_outcome_review_receipt_v1"
);
const outRoot = resolve(
  argValue(
    "--out-dir",
    join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-separate-real-runner-outcome-reviews")
  )
);
...
const forbiddenDecisions = new Set([
  "write_memory",
  "enable_rule",
  "activate_durable_memory",
  "activate_durable_rule",
...
if (runner.status !== "real_case_confirmed_outcome_separate_real_runner_completed_waiting_for_teacher_outcome_review") {
  block(
    "runner_not_waiting_for_outcome_review",
    "Confirmed-outcome runner must have completed one controlled attempt and be waiting for teacher outcome review."
  );
}
...
if (runner.locks?.confirmedOutcomeBranch !== true || runner.separateRealRunnerRequest?.confirmedOutcomeBranch !== true) {
  block("confirmed_outcome_branch_missing", "Source runner must retain confirmedOutcomeBranch=true.");
}
if (runner.confirmedOutcomeBranch !== true) {
  block("runner_confirmed_outcome_branch_missing", "Source runner packet must retain confirmedOutcomeBranch=true.");
}
if (runner.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT) {
  block("runner_source_review_format_mismatch", "Source runner must retain the confirmed-outcome outcome review source format.");
}
if (!runner.sourceConfirmedOutcomeReviewId || !runner.sourceConfirmedOutcomeSourceRunId || !runner.sourceRunId) {
  block(
    "runner_confirmed_outcome_source_ids_missing",
    "Source runner must retain sourceConfirmedOutcomeReviewId, sourceConfirmedOutcomeSourceRunId, and sourceRunId."
  );
}
...
if (runner.locks?.ruleEnabled !== false || runner.locks?.accepted !== false || runner.locks?.goalComplete !== false) {
  block("source_runner_acceptance_locks_not_closed", "Source runner must keep rule, acceptance, and goal-completion locks closed.");
}
...
if (receipt.sourceReviewFormat !== runner.sourceReviewFormat) {
  block("receipt_source_review_format_mismatch", "Receipt sourceReviewFormat must match the source runner.");
}
if (receipt.sourceConfirmedOutcomeReviewId !== runner.sourceConfirmedOutcomeReviewId) {
  block("receipt_source_confirmed_outcome_review_id_mismatch", "Receipt sourceConfirmedOutcomeReviewId must match the source runner.");
}
if (receipt.sourceConfirmedOutcomeSourceRunId !== runner.sourceConfirmedOutcomeSourceRunId) {
  block(
    "receipt_source_confirmed_outcome_source_run_id_mismatch",
    "Receipt sourceConfirmedOutcomeSourceRunId must match the source runner sourceConfirmedOutcomeSourceRunId."
  );
}
...
if (
  ...
  runner.adapterRun?.outputSha256 &&
  String(runner.adapterRun.outputSha256).toLowerCase() !== hashFile(outputPath)
) {
...
const requiredConfirmations = [
  ...
  ["confirmedOutcomeBranchReviewed", "confirmed_outcome_branch_not_reviewed"],
  ...
  ["noGoalCompletionClaimConfirmed", "no_goal_completion_claim_not_confirmed"]
];
for (const [field, code] of requiredConfirmations) {
  if (receipt[field] !== true) block(code, `${field} must be true before confirmed-outcome review can route forward.`);
}
...
const runId = `${new Date().toISOString().replace(/[:.]/g, "-")}-${runner.runId || "confirmed-outcome-runner-review"}`;
const reviewDir = join(outRoot, runId.replace(/[\\/:*?"<>|]/g, "_"));
const reviewPath = join(reviewDir, "real-case-confirmed-outcome-separate-real-runner-outcome-review.json");
const receiptRecordPath = join(reviewDir, "teacher-confirmed-outcome-review-receipt.json");
const readmePath = join(reviewDir, "REAL_CASE_CONFIRMED_OUTCOME_SEPARATE_REAL_RUNNER_OUTCOME_REVIEW_START_HERE.md");
const locks = reviewLocks({ reviewed });

const status =
  blockers.length > 0
    ? "real_case_confirmed_outcome_separate_real_runner_outcome_review_blocked"
    : decision === "confirm_controlled_output_matches_intent"
      ...
      : decision === "request_high_reasoning_repair"
        ? "real_case_confirmed_outcome_separate_real_runner_outcome_routes_to_high_reasoning_repair"
        : "real_case_confirmed_outcome_separate_real_runner_outcome_review_needs_teacher_follow_up";
const sourceContext = {
  confirmedOutcomeBranch: runner.confirmedOutcomeBranch === true,
...
const highReasoningRepairHandoff =
  blockers.length === 0 && decision === "request_high_reasoning_repair"
    ? {
        format: "transparent_ai_real_case_confirmed_outcome_separate_real_runner_high_reasoning_repair_handoff_v1",
        confirmedOutcomeBranch: true,
        sourceReviewFormat: runner.sourceReviewFormat,
        sourceConfirmedOutcomeReviewId: runner.sourceConfirmedOutcomeReviewId,
        sourceConfirmedOutcomeSourceRunId: runner.sourceConfirmedOutcomeSourceRunId,
        ...
        nextAction: "repair_confirmed_outcome_logic_contract_or_runner_plan_before_any_new_execution",
        executeNow: false,
        memoryWriteAllowedHere: false,
        ruleEnableAllowedHere: false,
        productionRuleRegistryMutationAllowedHere: false,
...
const packet = {
  ...
  nextTeacherActions:
    blockers.length > 0
      ? ["Resolve the blockers before routing the confirmed-outcome runner result forward."]
      : decision === "confirm_controlled_output_matches_intent"
        ? ["Use a separate memory or rule activation gate if this confirmed outcome should become durable apprentice behavior."]
        : decision === "request_high_reasoning_repair"
          ? ["Send the handoff to a high-reasoning repair pass before any new confirmed-outcome execution."]
          : ["Collect the missing teacher evidence before confirmed-outcome routing."],
  locks,
  paths: {
    review: reviewPath,
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Confirmed-Outcome Separate Real Runner Outcome Review",
    "",
    `Status: ${status}`,
    `Decision: ${decision}`,
    ...
    "",
    "This gate reviews the result of one final teacher-confirmed confirmed-outcome runner attempt.",
    "It does not execute software again, write memory, fetch RAG, enable rules, mutate the rule registry, unlock packaging, accept technology, or complete the whole apprentice objective.",
    "",
    "Blockers:",
...
console.log(
  JSON.stringify(
    {
      ok: packet.ok,
      format: "transparent_ai_real_case_confirmed_outcome_separate_real_runner_outcome_review_result_v1",
      status,
      reviewPath,
      receiptRecordPath,
      readmePath,
