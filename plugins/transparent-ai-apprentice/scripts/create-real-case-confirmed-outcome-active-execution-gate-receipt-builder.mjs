...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-active-execution-gate-receipt-builders"))
);
...
if (
  ...
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_BUILDER_REQUIRES_CLOSED_ACTIVE_DELIVERY_GATE");
}
if (!gate.rollbackPoint || !existsSync(gate.rollbackPoint)) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_BUILDER_REQUIRES_RETAINED_ROLLBACK_POINT");
}
if (!gate.compiledActiveRulePackagePath || !existsSync(gate.compiledActiveRulePackagePath)) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_BUILDER_REQUIRES_ACTIVE_RULE_PACKAGE");
}
if (gate.confirmedOutcomeBranch !== true) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_SOURCE_BRANCH_MISSING");
}
if (gate.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_SOURCE_FORMAT_MISMATCH");
}
if (!gate.sourceConfirmedOutcomeReviewId || !gate.sourceConfirmedOutcomeSourceRunId || !gate.sourceRunId) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_SOURCE_IDS_MISSING");
}

const builderId = `${new Date().toISOString().replace(/[:.]/g, "-")}-${gate.caseType || "real_case_confirmed_outcome_active_execution_gate"}`;
const builderDir = join(outRoot, builderId);
const builderPath = join(builderDir, "real-case-confirmed-outcome-active-execution-gate-receipt-builder.json");
const receiptTemplatePath = join(builderDir, "real-case-confirmed-outcome-active-execution-gate-receipt-template.json");
const readmePath = join(builderDir, "REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_REVIEW_START_HERE.md");
const htmlPath = join(builderDir, "real-case-confirmed-outcome-active-execution-gate-review.html");
...
const receiptTemplate = {
  format: "transparent_ai_real_case_confirmed_outcome_active_execution_gate_receipt_v1",
...
const builder = {
  ok: true,
  format: "transparent_ai_real_case_confirmed_outcome_active_execution_gate_receipt_builder_v1",
  builderId,
  createdAt: new Date().toISOString(),
  status: "ready_for_teacher_real_case_confirmed_outcome_active_execution_gate_review",
  sourceDeliveryGateId: gate.gateId,
  sourceDeliveryGatePath: gateInput.path,
  sourceDeliveryGateHash: gateHash,
  ...sourceContext,
  ...
  nextValidationCommand:
    'node plugins\\transparent-ai-apprentice\\scripts\\validate-real-case-confirmed-outcome-active-execution-gate-receipt.mjs --delivery-gate "' +
    (gateInput.path || "<real-case-confirmed-outcome-active-delivery-gate.json>") +
    '" --receipt "<teacher-filled-real-case-confirmed-outcome-active-execution-gate-receipt.json>"',
  locks: builderLocks,
  paths: {
    builder: builderPath,
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Confirmed-Outcome Active Execution Gate Review",
    "",
    "This is the teacher checkpoint after a closed confirmed-outcome active delivery gate. It may approve only a controlled execution request for a later separate runner.",
    "",
...
writeFileSync(
  ...
  `<!doctype html>
<html>
<head><meta charset="utf-8"><title>Real-Case Confirmed-Outcome Active Execution Gate Review</title></head>
<body>
<h1>Real-Case Confirmed-Outcome Active Execution Gate Review</h1>
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_real_case_confirmed_outcome_active_execution_gate_receipt_builder_result_v1",
