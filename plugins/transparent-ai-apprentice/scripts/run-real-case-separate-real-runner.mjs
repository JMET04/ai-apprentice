...
function finalConfirmationMatches(value) {
  ...
  return [
    ...
    "teacher confirmed separate real runner",
    "approve real-case separate real runner",
    "i confirm real-case separate real runner"
...
function runReviewedNodeManifest(manifest, outputDir) {
  ...
  const targetOutputFileName = safeFileName(manifest.targetOutputFileName, "real-case-runner-output.json");
...
const outRoot = resolve(argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-separate-real-runs")));
...
if (gate.status !== "real_case_adapter_specific_runner_approval_gate_ready_for_separate_real_runner") {
  block("approval_gate_not_ready", "Approval gate must be ready for separate real runner.");
}
if (gate.approvedForSeparateRealRunner !== true) block("approval_gate_not_approved", "Approval gate must be approved for separate real runner.");
if (!request || request.format !== "transparent_ai_real_case_separate_real_runner_request_v1") {
  block("separate_real_runner_request_missing", "Gate must include a separate real-runner request.");
}
...
if (
  ...
  block("source_gate_locks_not_closed", "Source approval gate must have closed execution locks.");
}

const runId = `${new Date().toISOString().replace(/[:.]/g, "-")}-${request?.adapterSelection?.adapterId || "real-case-runner"}`;
...
const runPath = join(runDir, "real-case-separate-real-runner.json");
const receiptPath = join(runDir, "real-case-separate-real-runner-receipt.json");
const readmePath = join(runDir, "REAL_CASE_SEPARATE_REAL_RUNNER_START_HERE.md");
...
const status =
  blockers.length > 0 && !runnerInvoked
    ? "blocked_before_real_case_separate_real_runner"
    : controlledRouteActionExecuted
      ? "real_case_separate_real_runner_completed_waiting_for_teacher_outcome_review"
      : runnerInvoked
        ? "real_case_separate_real_runner_invoked_waiting_for_teacher_review"
        : "blocked";
const runnerLocks = locks({ runnerInvoked });
const packet = {
  ok: blockers.length === 0,
  format: "transparent_ai_real_case_separate_real_runner_v1",
  runId,
  ...
  nextTeacherActions: controlledRouteActionExecuted
    ? [
        "Review the real-case separate runner packet and receipt.",
        "Inspect the controlled output file and hash.",
        "If the output is wrong, route the correction to high-reasoning repair before memory or rule enablement."
      ]
    : [
        "Resolve every blocker before any real-case separate runner attempt.",
        "Keep the fresh rollback point until the teacher confirms the run direction is correct."
      ],
  completionBoundary: {
    goalComplete: false,
    accepted: false,
    reason: "This is one controlled real-case runner attempt. It does not accept technology, write memory, unlock packaging, or complete the whole apprentice objective."
  },
  locks: runnerLocks,
  paths: {
    run: runPath,
...
const receipt = {
  format: "transparent_ai_real_case_separate_real_runner_receipt_v1",
  runId,
  status,
  sourceApprovalGatePath: gateInput.path,
  freshRollbackPoint,
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Separate Real Runner",
    "",
    `Status: ${status}`,
    `Runner invoked: ${runnerInvoked}`,
    `Controlled route action executed: ${controlledRouteActionExecuted}`,
    "",
    "This runner consumes one adapter-specific approval gate, a fresh rollback point, final teacher confirmation, and a teacher-reviewed runner manifest.",
...
console.log(
  JSON.stringify(
    {
      ok: packet.ok,
      format: "transparent_ai_real_case_separate_real_runner_result_v1",
      status,
      runPath,
      receiptPath,
      readmePath,
