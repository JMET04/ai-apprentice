...
function slug(value) {
  return (
    String(value || "real-case-validation-report-review")
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 72) || "real-case-validation-report-review"
  );
}
...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-validation-report-review-receipt-builders"))
);
if (!packetPath) {
  throw new Error(
    "Usage: node create-real-case-validation-report-review-receipt-builder.mjs --report-packet <real-case-disabled-package-validation-report-packet.json> [--out-dir <dir>]"
  );
}
if (!existsSync(packetPath)) throw new Error(`REAL_CASE_VALIDATION_REPORT_PACKET_NOT_FOUND: ${packetPath}`);

const packet = readJson(packetPath);
if (packet.format !== "transparent_ai_real_case_disabled_package_validation_report_v1") {
  throw new Error("Expected transparent_ai_real_case_disabled_package_validation_report_v1.");
}
if (
  packet.status !== "ready_for_teacher_real_case_validation_report_review" ||
...
const builderPath = join(builderDir, "real-case-validation-report-review-receipt-builder.json");
const receiptTemplatePath = join(builderDir, "real-case-validation-report-review-receipt-template.json");
const readmePath = join(builderDir, "REAL_CASE_VALIDATION_REPORT_REVIEW_START_HERE.md");
const htmlPath = join(builderDir, "real-case-validation-report-review.html");
const builderLocks = locks();

const receiptTemplate = {
  format: "transparent_ai_real_case_validation_report_review_receipt_v1",
...
const builder = {
  ok: true,
  format: "transparent_ai_real_case_validation_report_review_receipt_builder_v1",
  builderId,
  ...
  nextValidationCommand:
    'node plugins\\transparent-ai-apprentice\\scripts\\validate-real-case-validation-report-review-receipt.mjs --report-packet "' +
    packetPath +
    '" --receipt "<teacher-filled-real-case-validation-report-review-receipt.json>"',
  blockedActions: [
...
writeFileSync(
  readmePath,
  [
    "# Real Case Validation Report Review",
    "",
...
<head>
  ...
  <title>Real Case Validation Report Review</title>
  <style>
...
<body>
  <main>
    <h1>Real Case Validation Report Review</h1>
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_real_case_validation_report_review_receipt_builder_result_v1",
      builderPath,
      receiptTemplatePath,
      readmePath,
      htmlPath,
