...
function slugify(value) {
  return (
    String(value || "execution-follow-up-handoff-item-command-builder")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "")
      .slice(0, 72) || "execution-follow-up-handoff-item-command-builder"
  );
}
...
function normalizedItems(queue) {
  const items = (Array.isArray(queue?.queueItems) ? queue.queueItems : []).filter(
    (item) => item.kind === "reviewed_dry_run_runner_command"
  );
  if (!items.length) {
    return [
      {
        id: "placeholder_execution_follow_up_item_001",
        number: 1,
        rowId: "",
        label: "Placeholder item until a validated execution follow-up handoff queue is selected",
        kind: "placeholder",
        status: "waiting_for_handoff_queue",
        placeholders: ["<execution-follow-up-handoff-queue.json>"],
  ...
  return items.map((item, index) => ({
    id: item.id || `execution_follow_up_item_${String(index + 1).padStart(3, "0")}`,
    number: item.number || index + 1,
    rowId: item.rowId || "",
    label: item.label || item.tool || `Execution follow-up item ${index + 1}`,
    kind: item.kind || "",
    status: item.status || "",
...
function writeReadme(path, builder) {
  const lines = [
    "# Execution Follow-Up Handoff Item Command Builder",
    "",
    `Status: ${builder.status}`,
    `Queue: ${builder.paths.sourceQueue || "<queue path not loaded yet>"}`,
    `Ready dry-run items: ${builder.counts.readyDryRunItems}`,
    "",
    "Use the HTML page to pick exactly one teacher-reviewed execution dry-run handoff item, add the retained rollback point, and copy the generated single-item runner command.",
    "",
    ...
    "- It does not run the command, read logs, open GUI files, register tasks, launch runners, execute target software, send UI events, capture screenshots, write memory, accept rules, unlock packaging, or claim completion.",
    "- The generated command invokes the dry-run-only structured handoff item runner; it does not execute the queue display command string."
  ];
  ...
  const rows = builder.items
    .map(
      (item) => `<tr>
        ...
        <td><code>${htmlEscape(item.rowId)}</code></td>
...
<head>
  ...
  <title>Execution Follow-Up Handoff Item Command Builder</title>
  <style>
...
<body>
  <main>
    <h1>Execution Follow-Up Handoff Item Command Builder</h1>
    ...
    <p class="lock">This page only generates command text. It does not run commands, read logs, register tasks, capture screenshots, send UI events, write memory, execute target software, unlock packaging, or claim completion.</p>
    <section class="panel">
      <label>Queue path
        <input id="queuePath" value="${htmlEscape(builder.paths.sourceQueue || "<execution-follow-up-handoff-queue.json>")}">
      </label>
      <label>Teacher confirmation
        <input id="teacherConfirmation" value="teacher confirmed execution follow-up dry-run item">
      </label>
      <label>Retained rollback point
        <input id="rollbackPoint" value="<retained-rollback-point-path-or-label>">
      </label>
      <div class="controls">
        <button id="generateCommand">Generate dry-run item command</button>
    ...
    <table>
      <thead><tr><th>Pick</th><th>#</th><th>Row ID</th><th>Status</th><th>Label</th><th>Placeholders</th><th>Safe</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </main>
  <script>
    ...
    function selectedItem() {
      const number = document.querySelector('input[name="item"]:checked')?.value || "1";
      return builder.items.find((item) => String(item.number) === String(number)) || builder.items[0];
    }
    function makeCommand() {
      ...
      const request = {
        format: "transparent_ai_execution_follow_up_handoff_item_run_request_v1",
        generatedBy: "execution_follow_up_handoff_item_command_builder",
        queuePath,
        itemNumber: String(item.number || "1"),
        rowId: item.rowId || "",
        dryRunOnly: true,
      ...
      const selector = request.rowId ? ["--row-id", q(request.rowId)] : ["--item-number", q(request.itemNumber)];
      request.command = [
        "node plugins\\\\transparent-ai-apprentice\\\\scripts\\\\run-all-software-execution-follow-up-handoff-queue-item.mjs",
        "--queue", q(queuePath),
        ...selector,
    ...
    document.getElementById("downloadRequest").addEventListener("click", () => {
      ...
      link.download = "execution-follow-up-handoff-item-run-request.json";
      link.click();
...
const goal = argValue("--goal", "Build a teacher-facing command page for one execution follow-up dry-run handoff item.");
...
const outputRoot = resolve(
  argValue("--output-dir", join(process.cwd(), ".transparent-apprentice", "execution-follow-up-handoff-item-command-builders"))
);
...
const builderPath = join(builderDir, "execution-follow-up-handoff-item-command-builder.json");
const htmlPath = join(builderDir, "execution-follow-up-handoff-item-command-builder.html");
const readmePath = join(builderDir, "EXECUTION_FOLLOW_UP_HANDOFF_ITEM_COMMAND_BUILDER_START_HERE.md");
const items = normalizedItems(queue);
const builderLocks = locks();
const readyDryRunItems = items.filter(
  (item) =>
...
const builder = {
  ok: true,
  format: "transparent_ai_execution_follow_up_handoff_item_command_builder_v1",
  builderId,
  createdAt: new Date().toISOString(),
  goal,
  status: queue ? "waiting_for_teacher_single_execution_follow_up_item_command_generation" : "waiting_for_handoff_queue_path",
  queueSupported: Boolean(queue),
  sourceQueueStatus: queue?.status || "",
  sourceQueueDecision: queue?.queueDecision || "",
  counts: {
    queueItems: items.length,
    readyDryRunItems: readyDryRunItems.length,
  ...
  items,
  commandTemplate: commandLine("run-all-software-execution-follow-up-handoff-queue-item.mjs", [
    ["--queue", queueInput.path || "<execution-follow-up-handoff-queue.json>"],
    ["--row-id", "<teacher-reviewed-row-id>"],
    ["--teacher-confirmation", "<teacher-confirmed-execution-follow-up-dry-run-item-text>"],
    ["--rollback-point-created", "true"],
    ["--rollback-point", "<retained-rollback-point-path-or-label>"]
  ]),
  blockedActions: [
    "execute_queue_display_command_string",
    "invoke_execution_follow_up_runner_without_teacher_selection",
    "invoke_multi_item_execution_follow_up_from_command_builder",
    "pass_execute_flag_from_command_builder",
    "read_logs_from_command_builder",
    "capture_screenshot_from_command_builder",
    "register_schedule_from_command_builder",
    "send_ui_events_from_command_builder",
    "execute_target_software_from_command_builder",
    "write_memory_from_command_builder",
    "claim_native_universal_execution_from_command_builder",
