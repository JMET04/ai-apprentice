...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-active-promotion-review-builders"))
);
...
if (transitionPackage.confirmedOutcomeBranch !== true) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_PROMOTION_SOURCE_BRANCH_MISSING");
}
if (transitionPackage.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_PROMOTION_SOURCE_FORMAT_MISMATCH");
}
if (!transitionPackage.sourceConfirmedOutcomeReviewId || !transitionPackage.sourceConfirmedOutcomeSourceRunId || !transitionPackage.sourceRunId) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_PROMOTION_SOURCE_IDS_MISSING");
}
...
const builderPath = join(builderDir, "real-case-confirmed-outcome-active-promotion-review-receipt-builder.json");
const receiptTemplatePath = join(builderDir, "real-case-confirmed-outcome-active-promotion-review-receipt-template.json");
const readmePath = join(builderDir, "REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_PROMOTION_REVIEW_START_HERE.md");
const htmlPath = join(builderDir, "real-case-confirmed-outcome-active-promotion-review.html");
...
const receiptTemplate = {
  format: "transparent_ai_real_case_confirmed_outcome_active_promotion_review_receipt_v1",
...
const builder = {
  ok: true,
  format: "transparent_ai_real_case_confirmed_outcome_active_promotion_review_receipt_builder_v1",
  builderId,
  createdAt: new Date().toISOString(),
  status: "ready_for_teacher_real_case_confirmed_outcome_active_promotion_review",
  sourceTransitionPackageId: transitionPackage.transitionId,
  sourceTransitionPackagePath: transitionInput.path,
  sourceTransitionPackageHash: transitionHash,
  ...sourceContext,
  ...
  nextValidationCommand:
    'node plugins\\transparent-ai-apprentice\\scripts\\validate-real-case-confirmed-outcome-active-promotion-review-receipt.mjs --transition-package "' +
    (transitionInput.path || "<real-case-confirmed-outcome-review-only-transition-package.json>") +
    '" --receipt "<teacher-filled-real-case-confirmed-outcome-active-promotion-review-receipt.json>"',
  locks: builderLocks,
  paths: {
    builder: builderPath,
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Confirmed Outcome Active Promotion Review",
    "",
...
writeFileSync(
  ...
  `<!doctype html>
<html>
<head><meta charset="utf-8"><title>Real-Case Confirmed Outcome Active Promotion Review</title></head>
<body>
<h1>Real-Case Confirmed Outcome Active Promotion Review</h1>
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_real_case_confirmed_outcome_active_promotion_review_receipt_builder_result_v1",
