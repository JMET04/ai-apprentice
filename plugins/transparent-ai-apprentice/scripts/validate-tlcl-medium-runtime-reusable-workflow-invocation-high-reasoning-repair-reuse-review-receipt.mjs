...
function slug(value) {
  return (
    String(value || "tlcl-reusable-workflow-repair-reuse-review-validation")
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 72) || "tlcl-reusable-workflow-repair-reuse-review-validation"
  );
}
...
function adaptRepairReuseReceipt(receipt) {
  return {
    ...receipt,
    format: "transparent_ai_tlcl_medium_runtime_reusable_workflow_activation_receipt_v1",
    candidateReviewed: receipt.candidateReviewed === true || receipt.repairOutcomeValidationReviewed === true,
    contractBoundaryReviewed: receipt.contractBoundaryReviewed === true || receipt.repairedWorkflowBoundaryReviewed === true,
    sourceOutcomeEvidenceReviewed: receipt.sourceOutcomeEvidenceReviewed === true || receipt.repairOutcomeValidationReviewed === true,
...
const goal = argValue("--goal", "Validate teacher review for retaining a repaired reusable workflow.");
const candidateInput = readJsonInput(
  argValue("--candidate", argValue("--repair-reuse-candidate", argValue("--workflow-candidate", ""))),
  "--candidate",
...
const receiptInput = readJsonInput(
  ...
  "--receipt",
  "transparent_ai_tlcl_medium_runtime_reusable_workflow_invocation_high_reasoning_repair_reuse_review_receipt_v1"
);
const outRoot = resolve(
  argValue(
    "--out-dir",
    join(process.cwd(), ".transparent-apprentice", "tlcl-reusable-workflow-repair-reuse-review-validations")
  )
);
...
writeJson(adaptedReceiptPath, adaptRepairReuseReceipt(receipt));

const preflightBlockers = [];
if (candidate.status !== "reusable_workflow_repair_reuse_review_candidate_ready_for_teacher_review") {
  preflightBlockers.push("repair_reuse_review_candidate_not_ready");
}
...
if (receipt.repairOutcomeValidationReviewed !== true && receipt.teacherDecision === "approve_medium_runtime_reuse") {
  preflightBlockers.push("repair_outcome_validation_not_reviewed");
}
...
if (ragInformedRepairReuse) {
  if (candidate.ragEvidenceTreatedAsAuthority !== false) preflightBlockers.push("rag_informed_repair_reuse_candidate_treats_rag_as_authority");
  if (candidate.ragEvidenceNonAuthoritative !== true) preflightBlockers.push("rag_informed_repair_reuse_candidate_non_authority_flag_missing");
  if (candidate.locks?.ragEvidenceNonAuthoritative !== true) preflightBlockers.push("rag_informed_repair_reuse_candidate_non_authority_lock_missing");
  if (candidate.locks?.doesNotTreatRagAsAuthority !== true) {
    preflightBlockers.push("rag_informed_repair_reuse_candidate_does_not_treat_rag_as_authority_lock_missing");
  }
  if (receipt.ragEvidenceTreatedAsAuthority !== false) preflightBlockers.push("rag_informed_reuse_review_receipt_treats_rag_as_authority");
  if (receipt.ragEvidenceNonAuthoritative !== true) preflightBlockers.push("rag_informed_reuse_review_receipt_non_authority_flag_missing");
  if (receipt.ragEvidenceNonAuthoritativeConfirmed !== true && receipt.teacherDecision === "approve_medium_runtime_reuse") {
    preflightBlockers.push("rag_informed_reuse_review_non_authority_not_confirmed");
  }
}
...
const status = forbiddenDecisionUsed
  ? "blocked_for_forbidden_repaired_reusable_workflow_reuse_review_decision"
  : mediumRuntimeWorkflowEnabled
    ...
    : escalateToHighReasoningRepair
      ...
      : "repaired_reusable_workflow_reuse_review_needs_teacher_review_or_more_evidence";
...
const highReasoningRepairHandoff = escalateToHighReasoningRepair
  ? {
      kind: "repaired_reusable_workflow_reuse_review_high_reasoning_repair_handoff",
      runtimeTransition: "repair_reuse_review_to_high_reasoning_contract_repair",
      workflowFingerprint: candidate.workflowFingerprint || "",
      providerRoleUsePlanTrace,
      reasoningBudgetGovernorReviewTrace,
      ragInformedRepairReuse,
      ...
      evidenceToInspect: [
        candidateInput.path,
        candidate.sourceEvidence?.repairOutcomeValidationPath || "",
        candidate.sourceEvidence?.reusedCandidatePath || "",
  ...
  : null;
const validationPath = join(validationDir, "tlcl-reusable-workflow-repair-reuse-review-validation.json");
const validationReceiptPath = join(validationDir, "tlcl-reusable-workflow-repair-reuse-review-validation-receipt.json");
const readmePath = join(validationDir, "TLCL_REUSABLE_WORKFLOW_REPAIR_REUSE_REVIEW_VALIDATION_START_HERE.md");
const validation = {
  ok: true,
  ...
  sourceEvidence: {
    repairReuseReviewCandidatePath: candidateInput.path,
    repairReuseReviewCandidateHash: sha256Object(candidate),
    repairReuseReviewReceiptPath: receiptInput.path,
    repairReuseReviewReceiptHash: sha256Object(receipt),
  ...
  blockedTransitions: [
    "run_repaired_reusable_workflow_from_reuse_review_validation",
    "execute_target_software_from_repair_reuse_review_validation",
    "write_memory_from_repair_reuse_review_validation",
    "enable_rule_from_repair_reuse_review_validation",
    "unlock_packaging_from_repair_reuse_review_validation",
    "treat_rag_as_authority_from_repair_reuse_review",
    "claim_goal_complete_from_repair_reuse_review_validation"
  ],
  paths: {
...
const validationReceipt = {
  format: "transparent_ai_tlcl_medium_runtime_reusable_workflow_invocation_high_reasoning_repair_reuse_review_validation_receipt_v1",
  validationId,
  status,
  decision: existingValidation.decision,
  ragInformedRepairReuse,
...
writeFileSync(
  readmePath,
  [
    "# TLCL Reusable Workflow Repair Reuse Review Validation",
    "",
    `Status: ${status}`,
    `Decision: ${existingValidation.decision}`,
    "",
    "This validation may allow a repaired reusable workflow to be used by a later invocation planner, but it does not run workflow code, execute target software, write memory, enable rules, unlock packaging, or claim completion.",
    "",
    "Blockers:",
  ...
  JSON.stringify(
    {
      ok: true,
      format:
        "transparent_ai_tlcl_medium_runtime_reusable_workflow_invocation_high_reasoning_repair_reuse_review_validation_result_v1",
      validationId,
      status,
      decision: existingValidation.decision,
      mediumRuntimeWorkflowEnabled,
