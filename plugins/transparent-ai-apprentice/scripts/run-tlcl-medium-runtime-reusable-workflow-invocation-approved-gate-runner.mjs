...
function slugify(value) {
  return (
    String(value || "tlcl-reusable-workflow-invocation-approved-gate-runner")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "")
      .slice(0, 72) || "tlcl-reusable-workflow-invocation-approved-gate-runner"
  );
}
...
function explicitFinalConfirmation(value) {
  ...
  return [
    ...
    "teacher confirmed reusable workflow approved gate runner",
    ...
    "i confirm tlcl reusable workflow approved gate runner"
...
function reasoningBudgetGovernorReviewTraceFromBuilder(builder) {
  return builder?.sourceEvidence?.reasoningBudgetGovernorReviewTrace || {};
}

const goal = argValue("--goal", argValue("--task", "Run one TLCL reusable workflow teacher-approved execution gate request."));
const builderInput = readJsonInput(
  argValue("--builder", argValue("--command-builder", argValue("--tlcl-command-builder", ""))),
  "--builder",
...
const outputRoot = resolve(
  argValue("--output-dir", join(process.cwd(), ".transparent-apprentice", "tlcl-reusable-workflow-invocation-approved-gate-runs"))
);
...
const blockers = [];
if (builder.status !== "reusable_workflow_invocation_approved_gate_command_builder_ready_for_teacher_final_confirmation") {
  blockers.push("reusable_workflow_command_builder_not_ready_for_final_confirmation");
}
...
if (builder.locks?.wrapperDoesNotRunApprovedGate !== true) blockers.push("reusable_workflow_command_builder_execution_lock_missing");
...
if (!finalConfirmation) blockers.push("missing_final_teacher_reusable_workflow_approved_gate_confirmation");
if (!rollbackPointCreated) blockers.push("rollback_point_not_confirmed_for_reusable_workflow_approved_gate_run");

let runnerInvoked = false;
let runnerResult = null;
const existingRunnerOutputRoot = join(process.cwd(), ".ta", "rwagr", runId.slice(0, 24));
if (blockers.length === 0) {
  runnerInvoked = true;
  const runnerTeacherConfirmation = finalConfirmation
    ? "teacher confirmed approved execution gate runner"
  ...
  if (!run.ok) {
    ...
    if (runnerResult.runnerInvoked !== true || runnerResult.controlledRouteActionExecuted !== true) {
      for (const blocker of runnerResult.blockers || ["approved_gate_runner_did_not_execute_controlled_route"]) {
        blockers.push(`approved_gate_runner_blocker: ${blocker}`);
      }
    }
  }
}
...
const status =
  blockers.length > 0 && !runnerInvoked
    ...
    : controlledRouteActionExecuted
      ...
      : runnerInvoked
        ? "reusable_workflow_approved_gate_runner_invoked_but_blocked"
        : "blocked";
const packetPath = join(runDir, "tlcl-reusable-workflow-invocation-approved-gate-runner.json");
const receiptPath = join(runDir, "tlcl-reusable-workflow-invocation-approved-gate-runner-receipt.json");
const readmePath = join(runDir, "TLCL_REUSABLE_WORKFLOW_INVOCATION_APPROVED_GATE_RUNNER_START_HERE.md");
const packet = {
  ok: true,
  ...
  status,
  sourceEvidence: {
    reusableWorkflowCommandBuilderPath: builderInput.path,
    approvalGatePath,
    workflowFingerprint: builder.sourceEvidence?.workflowFingerprint || "",
  ...
  nextTeacherActions: controlledRouteActionExecuted
    ? [
        ...
        "Decide whether this one reusable workflow invocation matched the TLCL contract.",
        "If wrong, return correction to high-reasoning contract repair before memory or rule enablement."
      ]
    : [
        "Resolve every blocker before invoking the reusable workflow approved gate runner.",
        "Use only a ready reusable workflow command builder with final teacher confirmation and rollback evidence."
      ],
  completionBoundary: {
    goalComplete: false,
    allSoftwareExecutionComplete: false,
    nativeUniversalExecution: false,
    reason: "This wrapper can execute only one teacher-approved reusable workflow controlled route. It does not prove universal execution or complete the apprentice."
  },
  paths: {
    packet: packetPath,
    receipt: receiptPath,
...
const receipt = {
  ok: true,
  format: "transparent_ai_tlcl_medium_runtime_reusable_workflow_invocation_approved_gate_runner_receipt_v1",
  runId,
  status,
  runnerInvoked,
  controlledRouteActionExecuted,
...
writeFileSync(
  readmePath,
  [
    "# TLCL Reusable Workflow Invocation Approved Gate Runner",
    "",
    `Status: ${status}`,
    `Runner invoked: ${runnerInvoked ? "yes" : "no"}`,
    `Controlled route action executed: ${controlledRouteActionExecuted ? "yes" : "no"}`,
    "",
    "This wrapper invokes the existing approved-gate runner only for one ready reusable workflow command builder with final teacher confirmation and rollback evidence.",
    "",
    "Blockers:",
