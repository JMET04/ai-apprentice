...
const smokeRoot = join(
  ...
  "tlcl-reusable-workflow-repair-approved-gate-command-builder-smoke",
  String(Date.now())
);
...
const rebuildSmoke = runNode("smoke-tlcl-medium-runtime-reusable-workflow-invocation-high-reasoning-repair-approval-gate-rebuild-package.mjs");
const ready = runNode("create-tlcl-medium-runtime-reusable-workflow-invocation-high-reasoning-repair-approved-gate-command-builder.mjs", [
  "--rebuild-package",
...
const blockedRebuildPath = writeJson(join(smokeRoot, "blocked-rebuild-package.json"), {
  ...
  status: "blocked_before_reusable_workflow_repair_approval_gate_rebuild",
  ...
  generatedEvidence: {},
  blockers: ["smoke_blocked_before_rebuild"],
  locks: {
    reviewOnly: true,
...
const blocked = runNode("create-tlcl-medium-runtime-reusable-workflow-invocation-high-reasoning-repair-approved-gate-command-builder.mjs", [
  "--rebuild-package",
  blockedRebuildPath,
  "--out-dir",
...
const checks = [
  {
    name: "Reusable workflow repair approved-gate command builder reuses only the existing command builder",
    pass:
      ready.status === "reusable_workflow_repair_approved_gate_command_builder_ready_for_teacher_final_confirmation" &&
  ...
  {
    name: "Reusable workflow repair approved-gate command builder blocks unready rebuild packages",
    pass:
      blocked.status === "blocked_before_reusable_workflow_repair_approved_gate_command_builder" &&
      blocked.reusableWorkflowCommandBuilderInvoked === false &&
      blockedWrapper.blockers.includes("repair_approval_gate_rebuild_package_not_ready_for_teacher_execute_review"),
    evidence: blockedWrapper.blockers.join(",")
  },
  {
    name: "Reusable workflow repair approved-gate command builder preserves RAG non-authority locks",
    pass:
      ready.ragInformedRepairReuse === true &&
      ready.ragEvidenceTreatedAsAuthority === false &&
      ready.ragEvidenceNonAuthoritative === true &&
      readyWrapper.ragInformedRepairReuse === true &&
      readyWrapper.ragEvidenceNonAuthoritative === true &&
      readyWrapper.locks.ragEvidenceNonAuthoritative === true &&
      readyWrapper.locks.doesNotTreatRagAsAuthority === true &&
      readyWrapper.blockedTransitions.includes("treat_rag_as_authority_from_repair_approved_gate_command_builder"),
    evidence: JSON.stringify(readyWrapper.sourceEvidence)
  },
  {
    name: "Reusable workflow repair approved-gate command builder preserves provider role-use trace from rebuild package",
    pass:
  ...
  {
    name: "Reusable workflow repair approved-gate command builder preserves reasoning budget trace from rebuild package",
    pass:
  ...
  {
    name: "Repair command builder keeps runner medium runtime execution memory rule packaging and completion locks",
    pass:
  ...
  {
    name: "Repair command builder output remains review-only and packaging gated",
    pass:
      readyWrapper.format ===
