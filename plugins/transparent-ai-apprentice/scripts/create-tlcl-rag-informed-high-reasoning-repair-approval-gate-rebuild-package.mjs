...
function slug(value) {
  return (
    String(value || "tlcl-rag-informed-repair-approval-gate-rebuild")
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 72) || "tlcl-rag-informed-repair-approval-gate-rebuild"
  );
}
...
function runPrepRunner(args, cwd) {
  ...
  if (result.status !== 0) return { ok: false, error: result.stderr || result.stdout || "approval gate prep runner failed" };
  return { ok: true, result: JSON.parse(result.stdout) };
}

function locks(approvalGateRebuilt = false) {
  return {
    reviewOnly: true,
    evidenceOnly: true,
...
const goal = argValue("--goal", "Rebuild one approval gate after RAG-informed TLCL repair fingerprint review.");
...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "tlcl-rag-informed-repair-approval-gate-rebuilds"))
);
...
const packagePath = join(rebuildDir, "tlcl-rag-informed-repair-approval-gate-rebuild-package.json");
const receiptPath = join(rebuildDir, "tlcl-rag-informed-repair-approval-gate-rebuild-receipt.json");
const readmePath = join(rebuildDir, "TLCL_RAG_INFORMED_REPAIR_APPROVAL_GATE_REBUILD_START_HERE.md");

const handoff = fingerprintReview.approvalGateRebuildHandoff || {};
const blockers = [];
if (fingerprintReview.status !== "rag_informed_fingerprint_review_ready_for_approval_gate_rebuild") {
  blockers.push("rag_informed_fingerprint_review_not_ready_for_approval_gate_rebuild");
}
if (fingerprintReview.readyForApprovalGateRebuild !== true) blockers.push("approval_gate_rebuild_ready_flag_missing");
if (handoff.kind !== "rag_informed_fingerprint_review_approval_gate_rebuild_handoff") {
  blockers.push("rag_informed_approval_gate_rebuild_handoff_missing");
}
...
if (fingerprintReview.locks?.ragEvidenceNonAuthoritative !== true) blockers.push("rag_non_authority_lock_missing_from_fingerprint_review");
if (fingerprintReview.locks?.doesNotEnableRules !== true) blockers.push("rule_enablement_lock_missing_from_fingerprint_review");
if (fingerprintReview.forbiddenDecisionUsed === true) blockers.push("forbidden_rag_informed_fingerprint_review_decision");
if (fingerprintReview.mediumRuntimeRetryAllowed !== false) blockers.push("medium_runtime_retry_must_stay_blocked");
if (plan.status !== "medium_runtime_reuse_invocation_ready_for_approval_gate_planning") {
  blockers.push("invocation_plan_not_ready_for_rag_informed_rebuilt_approval_gate");
}
...
if (blockers.length === 0) {
  ...
  if (!prep.ok) {
    ...
    if (!approvalGateRebuilt) {
      for (const blocker of prepResult.blockers || []) blockers.push(`rag_informed_approval_gate_rebuild_blocker:${blocker}`);
    }
  }
}
...
const status = approvalGateRebuilt
  ...
  : prepRunnerInvoked
    ? "rag_informed_repair_approval_gate_rebuild_prepared_but_blocked"
    : "blocked_before_rag_informed_repair_approval_gate_rebuild";
const packageLocks = locks(approvalGateRebuilt);
const rebuildPackage = {
  ...
  sourceEvidence: {
    ...
    deterministicValidationPackagePath: handoff.deterministicValidationPackagePath || "",
    validationReportPath: handoff.validationReportPath || "",
    attachmentPath: handoff.attachmentPath || "",
  ...
  nextTeacherActions: approvalGateRebuilt
    ? [
        ...
        "Confirm the RAG evidence is still non-authoritative and only supports the repaired logic.",
    ...
    : [
        ...
        "Return to high-reasoning repair if the RAG evidence, fingerprint review, plan, or route evidence no longer matches.",
        "Keep rollback points until the teacher confirms the rebuilt approval gate is correct."
      ],
  blockers,
  blockedTransitions: [
    "run_approved_gate_runner_from_rag_informed_repair_approval_gate_rebuild",
    "run_medium_runtime_workflow_from_rag_informed_repair_approval_gate_rebuild",
    "execute_target_software_from_rag_informed_repair_approval_gate_rebuild",
    "send_ui_events_from_rag_informed_repair_approval_gate_rebuild",
    "capture_screenshot_from_rag_informed_repair_approval_gate_rebuild",
    "write_memory_from_rag_informed_repair_approval_gate_rebuild",
    "enable_rule_from_rag_informed_repair_approval_gate_rebuild",
    "unlock_packaging_from_rag_informed_repair_approval_gate_rebuild",
    "treat_rag_as_authority_from_rag_informed_repair_approval_gate_rebuild",
    "claim_goal_complete_from_rag_informed_repair_approval_gate_rebuild"
  ],
  paths: {
    package: packagePath,
    receipt: receiptPath,
...
const receipt = {
  format: "transparent_ai_tlcl_rag_informed_high_reasoning_repair_approval_gate_rebuild_receipt_v1",
  rebuildId,
  status,
  approvalGateRebuilt,
  approvalGatePrepRunnerInvoked: prepRunnerInvoked,
...
writeFileSync(
  readmePath,
  [
    "# TLCL RAG-Informed Repair Approval Gate Rebuild",
    "",
    `Status: ${status}`,
    `Approval gate rebuilt: ${approvalGateRebuilt ? "yes" : "no"}`,
    "",
    "This package rebuilds only the approval gate after a teacher-approved RAG-informed high-reasoning repair fingerprint review.",
    ...
    "The package does not run the approved-gate runner, execute target software, send UI events, capture screenshots, write memory, enable rules, unlock packaging, or claim completion.",
    "",
    "Blockers:",
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_tlcl_rag_informed_high_reasoning_repair_approval_gate_rebuild_package_result_v1",
      rebuildId,
      status,
      approvalGateRebuilt,
      approvalGatePrepRunnerInvoked: prepRunnerInvoked,
