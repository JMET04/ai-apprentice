...
const outRoot = resolve(
  arg("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-active-validation-report-delivery-gates"))
);
const teacherReviewed = hasFlag("--teacher-reviewed");

if (!teacherReviewed) throw new Error("REAL_CASE_ACTIVE_VALIDATION_REPORT_DELIVERY_GATE_REQUIRES_TEACHER_REVIEWED_FLAG");
...
if (
  packet.status !== "ready_for_teacher_active_validation_report_delivery_gate_review" ||
  ...
  throw new Error("REAL_CASE_ACTIVE_DELIVERY_GATE_REQUIRES_ALLOWED_LOCKED_VALIDATION_REPORT_PACKET");
}
if (resolve(packet.rollbackPoint || "") !== rollbackPoint) {
  throw new Error("ROLLBACK_POINT_MISMATCH_FOR_ACTIVE_DELIVERY_GATE");
}

const validationReport = readJson(packet.validationReportPath);
if (validationReport.delivery_allowed !== true) {
  throw new Error("REAL_CASE_ACTIVE_DELIVERY_GATE_REJECTS_NON_DELIVERY_ALLOWED_VALIDATION_REPORT");
}
...
if (activeBlockingRows.length) {
  throw new Error("REAL_CASE_ACTIVE_DELIVERY_GATE_REJECTS_BLOCKING_VALIDATION_ROWS");
}
...
if (!activeRules.length || nonActiveRules.length || activeRules.length !== packet.activeRuleCount) {
  throw new Error("REAL_CASE_ACTIVE_DELIVERY_GATE_REQUIRES_MATCHING_ACTIVE_RULE_PACKAGE");
}

const gateId = stableId("real_case_active_validation_report_delivery_gate", `${validationReportInput.path || hashText(JSON.stringify(packet))}:${rollbackPoint}`);
const gateDir = join(outRoot, gateId);
const gatePath = join(gateDir, "real-case-active-validation-report-delivery-gate.json");
...
const gate = {
  ok: true,
  format: "transparent_ai_real_case_active_validation_report_delivery_gate_v1",
  gateId,
  createdAt: new Date().toISOString(),
  status: "active_delivery_gate_closed_ready_for_teacher_execution_gate_review",
  ...
  blockedTransitions: [
    "active_validation_delivery_allowed_to_rule_enabled",
    "active_validation_delivery_allowed_to_software_execution",
    "active_validation_delivery_allowed_to_memory_write",
    "active_validation_delivery_allowed_to_rag_fetch",
    "active_validation_delivery_allowed_to_packaging_unlock",
    "active_validation_delivery_allowed_to_technology_acceptance",
    "active_validation_delivery_allowed_to_goal_completion"
  ],
  summary: {
  ...
  nextReview: {
    instruction:
      "Review this closed active delivery gate before any separate execution gate. delivery_allowed is evidence only and cannot enable rules or execute target software here.",
    continueCondition:
      "Only continue to a separate execution gate if the teacher confirms the active Validation Report, rollback point, blocked transitions, and no-action locks.",
    stopCondition:
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_real_case_active_validation_report_delivery_gate_result_v1",
      status: gate.status,
      gatePath,
