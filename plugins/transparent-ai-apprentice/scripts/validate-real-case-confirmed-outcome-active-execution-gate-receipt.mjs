...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-active-execution-gate-validations"))
);
...
if (receipt.sourceDeliveryGateHash !== hashText(JSON.stringify(gate))) {
  block("source_delivery_gate_hash_mismatch", "Receipt sourceDeliveryGateHash must match the confirmed-outcome active delivery gate.");
}
if (gate.confirmedOutcomeBranch !== true) {
  block("source_confirmed_outcome_branch_missing", "Delivery gate must preserve confirmedOutcomeBranch=true.");
}
if (gate.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT) {
  block("source_review_format_mismatch", "Delivery gate must preserve the confirmed-outcome source review format.");
}
if (!gate.sourceConfirmedOutcomeReviewId || !gate.sourceConfirmedOutcomeSourceRunId || !gate.sourceRunId) {
  block("source_ids_missing", "Delivery gate must preserve confirmed-outcome review, source run, and run ids.");
}
...
if (receipt.sourceReviewFormat !== gate.sourceReviewFormat) {
  block("receipt_source_review_format_mismatch", "Receipt sourceReviewFormat must match the delivery gate.");
}
if (receipt.sourceConfirmedOutcomeReviewId !== gate.sourceConfirmedOutcomeReviewId) {
  block("receipt_source_review_id_mismatch", "Receipt sourceConfirmedOutcomeReviewId must match the delivery gate.");
}
if (receipt.sourceConfirmedOutcomeSourceRunId !== gate.sourceConfirmedOutcomeSourceRunId) {
  block(
    "receipt_source_confirmed_outcome_source_run_id_mismatch",
    "Receipt sourceConfirmedOutcomeSourceRunId must match the delivery gate."
  );
}
if (receipt.sourceRunId !== gate.sourceRunId) {
  block("receipt_source_run_id_mismatch", "Receipt sourceRunId must match the delivery gate.");
}
if (receipt.rollbackPoint && resolve(receipt.rollbackPoint) !== resolve(gate.rollbackPoint || "")) {
  block("rollback_point_mismatch", "Receipt rollbackPoint must match the confirmed-outcome active delivery gate rollbackPoint.");
}
...
if (
  gate.status !== "confirmed_outcome_active_delivery_gate_closed_ready_for_teacher_execution_gate_review" ||
  ...
  block("source_delivery_gate_not_locked", "Source confirmed-outcome active delivery gate must be closed and locked before execution review.");
}
...
if (!gate.compiledActiveRulePackagePath || !existsSync(gate.compiledActiveRulePackagePath)) {
  block("compiled_active_rule_package_not_found", "Compiled confirmed-outcome active Rule Package must still exist.");
}
...
if (decision === "approve_controlled_execution_request") {
  if (receipt.deliveryGateReviewed !== true) block("delivery_gate_not_reviewed", "Teacher must review the closed confirmed-outcome active delivery gate.");
  if (receipt.validationReportReviewed !== true) block("validation_report_not_reviewed", "Teacher must review the confirmed-outcome active Validation Report.");
  if (receipt.activeRulePackageReviewed !== true) block("active_rule_package_not_reviewed", "Teacher must review the confirmed-outcome active Rule Package.");
...
const status = forbiddenDecisionUsed
  ? "blocked_for_forbidden_real_case_confirmed_outcome_active_execution_gate_decision"
  : readyForControlledExecutionRequest
    ...
    : routesToHighReasoningRepair
      ? "real_case_confirmed_outcome_active_execution_gate_routes_to_high_reasoning_repair"
      : routesToMoreEvidence
        ? "real_case_confirmed_outcome_active_execution_gate_waiting_for_more_evidence"
        : "real_case_confirmed_outcome_active_execution_gate_needs_teacher_review";

const validationId = `${new Date().toISOString().replace(/[:.]/g, "-")}-${decision}`;
const validationDir = join(outRoot, validationId);
const validationPath = join(validationDir, "real-case-confirmed-outcome-active-execution-gate-validation.json");
const receiptRecordPath = join(validationDir, "real-case-confirmed-outcome-active-execution-gate-receipt.json");
const readmePath = join(validationDir, "REAL_CASE_CONFIRMED_OUTCOME_ACTIVE_EXECUTION_GATE_VALIDATION_START_HERE.md");
...
const highReasoningRepairHandoff = routesToHighReasoningRepair
  ? {
      format: "transparent_ai_real_case_confirmed_outcome_active_execution_gate_high_reasoning_repair_handoff_v1",
      sourceDeliveryGateId: gate.gateId,
      ...sourceContext,
...
const moreEvidenceHandoff = routesToMoreEvidence
  ? {
      format: "transparent_ai_real_case_confirmed_outcome_active_execution_gate_more_evidence_handoff_v1",
      sourceDeliveryGateId: gate.gateId,
      ...sourceContext,
      teacherNotes: receipt.teacherNotes || "",
...
const result = {
  ...
  format: "transparent_ai_real_case_confirmed_outcome_active_execution_gate_validation_v1",
  validationId,
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Confirmed-Outcome Active Execution Gate Validation",
    "",
    `Status: ${status}`,
    `Decision: ${decision}`,
    "",
...
console.log(
  JSON.stringify(
    {
      ok: result.ok,
      format: "transparent_ai_real_case_confirmed_outcome_active_execution_gate_validation_result_v1",
      status,
      validationPath,
      receiptRecordPath,
      readmePath,
