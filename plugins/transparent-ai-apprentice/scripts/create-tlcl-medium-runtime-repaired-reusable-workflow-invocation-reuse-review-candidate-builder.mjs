...
function slug(value) {
  return (
    String(value || "tlcl-repaired-reusable-workflow-invocation-reuse-review-candidate")
      .toLowerCase()
      .replace(/[^a-z0-9._-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 72) || "tlcl-repaired-reusable-workflow-invocation-reuse-review-candidate"
  );
}
...
function providerRoleUsePlanTraceFromValidation(validation) {
  return (
    validation.matchedOutcomeHandoff?.providerRoleUsePlanTrace ||
...
function reasoningBudgetGovernorReviewTraceFromValidation(validation) {
  return (
    validation.matchedOutcomeHandoff?.reasoningBudgetGovernorReviewTrace ||
...
function adaptRepairedInvocationOutcomeValidation(validation) {
  ...
  return {
    ...
    status:
      validation.status === "repaired_reusable_workflow_invocation_outcome_matched_contract_waiting_for_reuse_review"
    ...
    matchedContractHandoff: {
      ...
      sourceRunId: validation.matchedOutcomeHandoff?.sourceRunId || validation.matchedOutcomeHandoff?.sourceRepairedRunId || validation.validationId || "",
      executesNow: false,
      ...
      nextRequiredReview:
        "Teacher must review whether this repaired reusable workflow invocation remains safe to reuse with medium reasoning."
    },
    providerRoleUsePlanTrace,
    reasoningBudgetGovernorReviewTrace,
    sourceEvidence: {
...
const goal = argValue("--goal", "Build a reuse review candidate from one matched repaired reusable workflow invocation.");
const validationInput = readJsonInput(
  argValue("--validation", argValue("--repaired-outcome-validation", argValue("--outcome-review-validation", ""))),
  "--validation",
...
const outRoot = resolve(
  argValue(
    "--out-dir",
    join(process.cwd(), ".transparent-apprentice", "tlcl-repaired-reusable-workflow-invocation-reuse-review-candidates")
  )
);
...
writeJson(adaptedValidationPath, adaptRepairedInvocationOutcomeValidation(validation));

const blockers = [];
if (validation.status !== "repaired_reusable_workflow_invocation_outcome_matched_contract_waiting_for_reuse_review") {
  blockers.push("repaired_invocation_outcome_validation_not_matched_for_reuse_review");
}
if (validation.outcomeMatchedContract !== true) blockers.push("repaired_invocation_outcome_match_confirmation_missing");
if (!validation.matchedOutcomeHandoff) blockers.push("matched_repaired_invocation_outcome_handoff_missing");
if (validation.locks?.goalComplete !== false) blockers.push("repaired_invocation_outcome_completion_lock_missing");
if (validation.locks?.doesNotEnableRules !== true) blockers.push("repaired_invocation_outcome_rule_lock_missing");
if (ragInformedRepairReuse) {
  if (validation.ragEvidenceTreatedAsAuthority !== false) blockers.push("rag_informed_repaired_invocation_outcome_treats_rag_as_authority");
  if (validation.ragEvidenceNonAuthoritative !== true) blockers.push("rag_informed_repaired_invocation_outcome_non_authority_flag_missing");
  if (validation.locks?.ragEvidenceNonAuthoritative !== true) {
    blockers.push("rag_informed_repaired_invocation_outcome_non_authority_lock_missing");
  }
  if (validation.locks?.doesNotTreatRagAsAuthority !== true) {
    blockers.push("rag_informed_repaired_invocation_outcome_does_not_treat_rag_as_authority_lock_missing");
  }
}
...
const candidatePath = join(candidateDir, "tlcl-repaired-reusable-workflow-invocation-reuse-review-candidate.json");
const receiptTemplatePath = join(candidateDir, "tlcl-repaired-reusable-workflow-invocation-reuse-review-receipt-template.json");
const readmePath = join(candidateDir, "TLCL_REPAIRED_REUSABLE_WORKFLOW_INVOCATION_REUSE_REVIEW_CANDIDATE_START_HERE.md");
const status =
  blockers.length === 0
    ? "repaired_reusable_workflow_invocation_reuse_review_candidate_ready_for_teacher_review"
    : "blocked_before_repaired_reusable_workflow_invocation_reuse_review_candidate";
const receiptTemplate = existingTemplate
  ? {
      ...existingTemplate,
      format: "transparent_ai_tlcl_medium_runtime_repaired_reusable_workflow_invocation_reuse_review_receipt_v1",
      sourceRepairedInvocationReuseReviewCandidatePath: candidatePath,
      ragInformedRepairReuse,
      ragEvidenceTreatedAsAuthority: false,
      ragEvidenceNonAuthoritative: ragInformedRepairReuse,
      providerRoleUsePlanTrace,
      ...
      repairedInvocationOutcomeValidationReviewed: false,
      repairedWorkflowBoundaryReviewed: false,
      repairedWorkflowFingerprintReviewed: false,
      teacherNote:
        "Review whether this repaired reusable workflow invocation may be retained for later bounded medium-runtime invocation planning."
    }
  : {
      format: "transparent_ai_tlcl_medium_runtime_repaired_reusable_workflow_invocation_reuse_review_receipt_v1",
      candidateId,
...
const candidate = {
  ok: true,
  format: "transparent_ai_tlcl_medium_runtime_repaired_reusable_workflow_invocation_reuse_review_candidate_v1",
  candidateId,
  ...
  sourceEvidence: {
    repairedInvocationOutcomeValidationPath: validationInput.path,
    repairedInvocationOutcomeValidationHash: sha256Object(validation),
  ...
  blockedTransitions: [
    ...
    "write_memory_from_repaired_invocation_reuse_review_candidate",
    "enable_rule_from_repaired_invocation_reuse_review_candidate",
    "unlock_packaging_from_repaired_invocation_reuse_review_candidate",
    "treat_rag_as_authority_from_repaired_invocation_reuse_review_candidate",
    "claim_goal_complete_from_repaired_invocation_reuse_review_candidate"
  ],
  paths: {
...
writeFileSync(
  readmePath,
  [
    "# TLCL Repaired Reusable Workflow Invocation Reuse Review Candidate",
    "",
    `Status: ${status}`,
    `Workflow fingerprint: ${candidate.workflowFingerprint || "<blocked>"}`,
    "",
    "This wrapper reuses the existing bounded reusable workflow candidate builder after a repaired invocation fresh outcome matched the contract.",
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_tlcl_medium_runtime_repaired_reusable_workflow_invocation_reuse_review_candidate_builder_result_v1",
      candidateId,
      status,
