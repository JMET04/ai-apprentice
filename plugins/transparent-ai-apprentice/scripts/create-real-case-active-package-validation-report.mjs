...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-active-package-validation-reports"))
);
const teacherReviewed = hasFlag("--teacher-reviewed");

if (!teacherReviewed) throw new Error("REAL_CASE_ACTIVE_PACKAGE_VALIDATION_REPORT_REQUIRES_TEACHER_REVIEWED_FLAG");
...
if (
  ...
  throw new Error("Active compilation packet is not a locked handoff for active Validation Report evaluation.");
}
if (resolve(compilation.rollbackPoint || "") !== rollbackPoint) {
  throw new Error("ROLLBACK_POINT_MISMATCH_FOR_ACTIVE_VALIDATION_REPORT");
}
...
if (!activeRules.length) throw new Error("ACTIVE_VALIDATION_REPORT_REQUIRES_ACTIVE_RULES");
if (nonActiveRules.length) throw new Error("ACTIVE_VALIDATION_REPORT_REJECTS_NON_ACTIVE_RULES");

const reportId = stableId(
  "real_case_active_package_validation_report",
...
const validationReportPath = join(reportDir, "real-case-active-package-validation-report.json");
const packetPath = join(reportDir, "real-case-active-package-validation-report-packet.json");
...
const packet = {
  ok: true,
  format: "transparent_ai_real_case_active_package_validation_report_v1",
  reportId,
  ...
  nextReview: {
    instruction: deliveryAllowed
      ? "Review this active Validation Report before a separate delivery or execution gate. delivery_allowed is evidence only here."
      : "Review blocking active Validation Report rows and route repair before any delivery or execution gate.",
  ...
  blockedActions: [
    "open_delivery_gate_from_active_validation_report",
    "enable_rule_from_active_validation_report",
    "execute_software_from_active_validation_report",
    "write_memory_from_active_validation_report",
    "fetch_rag_from_active_validation_report",
    "unlock_packaging_from_active_validation_report",
    "claim_completion_from_active_validation_report"
  ],
...
console.log(
  JSON.stringify(
    {
      ok: true,
      format: "transparent_ai_real_case_active_package_validation_report_result_v1",
      status,
      packetPath,
      validationReportPath,
      deliveryAllowed,
