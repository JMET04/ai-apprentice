...
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-confirmed-outcome-review-only-package-planning"))
);
const teacherReviewed = hasFlag("--teacher-reviewed");

if (!teacherReviewed) throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PACKAGE_PLANNING_REQUIRES_TEACHER_REVIEWED_FLAG");
...
if (
  validation.status !== "confirmed_outcome_lifecycle_candidate_review_ready_for_review_only_package_planning" ||
  validation.readyForReviewOnlyPlanning !== true ||
  !handoff ||
  handoff.format !== "transparent_ai_real_case_confirmed_outcome_review_only_lifecycle_candidate_handoff_v1" ||
...
if (validation.confirmedOutcomeBranch !== true || handoff.confirmedOutcomeBranch !== true) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PLANNING_SOURCE_BRANCH_MISSING");
}
if (
  validation.sourceReviewFormat !== EXPECTED_SOURCE_REVIEW_FORMAT ||
  handoff.sourceReviewFormat !== validation.sourceReviewFormat
) {
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PLANNING_SOURCE_FORMAT_MISMATCH");
}
if (
  ...
  throw new Error("REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PLANNING_SOURCE_IDS_MISSING_OR_MISMATCHED");
}

const ruleDir = resolve(handoff.ruleDir || "");
if (!ruleDir || !existsSync(ruleDir)) throw new Error(`REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_RULE_DIR_NOT_FOUND: ${handoff.ruleDir || ""}`);
...
const planningId = `real_case_confirmed_outcome_review_only_package_plan.${new Date().toISOString().replace(/[:.]/g, "-")}`;
const planningDir = join(outRoot, planningId);
const packetPath = join(planningDir, "real-case-confirmed-outcome-review-only-package-planning-packet.json");
const readmePath = join(planningDir, "REAL_CASE_CONFIRMED_OUTCOME_REVIEW_ONLY_PACKAGE_PLANNING_START_HERE.md");
const htmlPath = join(planningDir, "real-case-confirmed-outcome-review-only-package-planning.html");
const planLocks = locks({ candidateCount: candidateRules.length });
const status = errors.length
  ? "blocked_real_case_confirmed_outcome_review_only_package_planning"
...
const packet = {
  ok: errors.length === 0,
  format: "transparent_ai_real_case_confirmed_outcome_review_only_package_planning_packet_v1",
  planningId,
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Confirmed Outcome Review-Only Package Planning",
    "",
    `Status: ${status}`,
...
writeFileSync(
  ...
  `<!doctype html>
<html>
<head><meta charset="utf-8"><title>Real-Case Confirmed Outcome Review-Only Package Planning</title></head>
<body>
<h1>Real-Case Confirmed Outcome Review-Only Package Planning</h1>
...
console.log(
  JSON.stringify(
    {
      ok: packet.ok,
      format: "transparent_ai_real_case_confirmed_outcome_review_only_package_planning_packet_result_v1",
      status,
      packetPath,
      readmePath,
      htmlPath,
