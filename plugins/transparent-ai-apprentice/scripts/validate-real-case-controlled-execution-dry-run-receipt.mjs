...
const receiptInput = readJsonInput(
  ...
  "--receipt",
  "transparent_ai_real_case_controlled_execution_dry_run_receipt_v1"
);
const outRoot = resolve(
  argValue("--out-dir", join(process.cwd(), ".transparent-apprentice", "real-case-adapter-specific-runner-approval-gates"))
);
...
if (receipt.sourceDryRunHash !== hashText(JSON.stringify(dryRun))) {
  block("source_dry_run_hash_mismatch", "Receipt sourceDryRunHash must match the dry-run packet.");
}
...
if (
  dryRun.status !== "real_case_controlled_execution_dry_run_ready_for_teacher_runner_review" ||
  ...
  block("source_dry_run_not_locked_ready", "Source dry-run must be ready, dry-run-only, and no-op locked.");
}
...
if (decision === "ready_for_separate_real_runner_gate") {
  if (receipt.dryRunReviewed !== true) block("dry_run_not_reviewed", "Teacher must review the controlled execution dry-run.");
...
const status = forbidden.has(decision)
  ? "blocked_for_forbidden_real_case_runner_gate_decision"
  : approved
    ? "real_case_adapter_specific_runner_approval_gate_ready_for_separate_real_runner"
    : routesToHighReasoningRepair
      ? "real_case_adapter_specific_runner_gate_routes_to_high_reasoning_repair"
      : routesToMoreEvidence
        ? "real_case_adapter_specific_runner_gate_waiting_for_more_evidence"
        : "real_case_adapter_specific_runner_gate_needs_teacher_review";
...
const gatePath = join(gateDir, "real-case-adapter-specific-runner-approval-gate.json");
const receiptRecordPath = join(gateDir, "real-case-controlled-execution-dry-run-receipt.json");
const readmePath = join(gateDir, "REAL_CASE_ADAPTER_SPECIFIC_RUNNER_APPROVAL_GATE_START_HERE.md");
const gateLocks = locks({ approved });

const separateRealRunnerRequest = approved
  ? {
      format: "transparent_ai_real_case_separate_real_runner_request_v1",
...
const result = {
  ...
  format: "transparent_ai_real_case_adapter_specific_runner_approval_gate_v1",
  gateId,
  ...
  blockedActions: [
    "invoke_adapter_from_approval_gate",
    "execute_target_software_command_from_approval_gate",
    "send_ui_events_from_approval_gate",
    "write_memory_from_approval_gate",
    "fetch_rag_from_approval_gate",
    "unlock_packaging_from_approval_gate",
    "claim_acceptance_from_approval_gate",
    "claim_completion_from_approval_gate"
  ],
  nextReview: {
    requiredDecision: approved ? "separate_real_runner_requires_final_teacher_execute_confirmation" : "teacher_review_required",
...
writeFileSync(
  readmePath,
  [
    "# Real-Case Adapter-Specific Runner Approval Gate",
    "",
    `Status: ${status}`,
    `Approved for separate real runner: ${approved}`,
    "",
    "This gate approves only a separate real-runner request with executeNow=false. It does not invoke adapters, send UI events, execute target software, write memory, fetch RAG, unlock packaging, accept technology, or claim completion.",
    "",
    `Gate JSON: ${gatePath}`,
    ""
  ].join("\n"),
...
console.log(
  JSON.stringify(
    {
      ok: result.ok,
      format: "transparent_ai_real_case_adapter_specific_runner_approval_gate_result_v1",
      status,
      gatePath,
      receiptRecordPath,
      readmePath,
