...
- 分别切换文字、图片和工程模式；
- 分别创建修改区、保护区和参考关系；
- 确认默认策略是“只做局部修改”，整体重生成必须单独选择；
- 工程模式填写对象 `D04`、动作“修改尺寸”、目标值 `450 mm`，确认其他对象被列为不可改动。

通过标准：导出包包含精确修改目标、保护区、参考关系、原生定位器和 `mingtu_surgical_edit_contract_v1`；未标注内容变化会被拒绝。

## 场景四 A：Word / Excel 原生点改
